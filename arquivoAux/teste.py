...
cc6=['alexLima_intelArtific_cc6','tatiane_matComputacional_cc6','felipeMaciel_sistemasDistribuidos_cc6','felipeMaciel_desenvWeb_cc6','bonfim_lingFormaisAutomatas_cc6', 'josimeire_empreendedorismo_cces']
cc7=['annaBeatriz_projetoPCeT_cces','bonfim_compiladores_cc7','bonfim_teoriaComp_cc7']
...
es6=['joaoVictor_verificValid_es6','josimeire_empreendedorismo_cces','annaBeatriz_manutSoft_es6','annaBeatriz_qualSoftware_es6', 'annaBeatriz_arquitSoft_es6']
...
rafelivo = ['rafaelIvo_introComp_cc1','rafaelIvo_lingProg_cces','rafaelIvo_sistemaOperacional_cc5','rafaelIvo_lingProg_cces']
    ...
    for cadeira in range(0, len(cadeiras)):
        for dia in range(0, len(dias)):
            for horarios in range(0, len(horariosAulas)):
                if(dias[dia] != "sexta" or horariosAulas[horarios] != "H4" or cadeiras[cadeira] != "bonfim_teoriaComp_cc7"):
                    gerarExistenciaCadeiras += [cadeiras[cadeira]+"_"+dias[dia]+"_"+horariosAulas[horarios]]
                else:
                    print(dias[dia])
                    gerarExistenciaCadeiras += [cadeiras[cadeira]+"_"+dias[dia]+"_"+horariosAulas[horarios]+'\n']
    info = open("testeTestes.txt", "w")
...
def garantiExistenciaDoisCreditos():
    ...
    info = open("existirDoisCredito.txt", "a")
    cleanList(str(gerarExistenciaCadeiras))
    info.write(cleanList(str(gerarExistenciaCadeiras)))
    info.close()
...
def gerarClausulasUmCredito():
    ...
    info = open('cadeirasUmCredito.txt', 'a')
                    ...
                    for horario2 in range(0, len(horariosAulas)):
                        if(dia1+2 == dia2 and horario1 == horario2):
                            ...
                            if(dia1 != dia2):
                                ...
                                cadeirasDoisCreditos += ["-"+cadeiras[cadeira]+"_"+dias[dia1]+"_"+horariosAulas[horario1]+" "+" *"+"-"+cadeiras[cadeira]+"_"+dias[dia2]+"_"+horariosAulas[horario2]]
    print(str(cadeirasDoisCreditos))
    info = open('cadeirasDoisCredito.txt', 'a')
    info.write(cleanList(str(cadeirasDoisCreditos)))
    info.close()
